...
We've had great feedback from our existing Boostrap [Blazor WASM template](/templates-blazor) enhanced with 
ServiceStack's highly [productive dev model](https://youtu.be/TIgjMf_vtCI) that integrates a pre-rendering solution
to achieve its [instant Startup performance](https://docs.servicestack.net/templates-blazor#improving-startup-performance) which
sees it become more popular than all our other jamstacks.net templates combined. 
...
[mobile-first responsive](https://tailwindcss.com/#mobile-first) websites built upon a beautiful expertly crafted constraint-based 
[Design System](https://tailwindcss.com/#constraint-based) that's enabled effortless reuse of a growing suite of [Free Community](https://tailwindcomponents.com)
...
with an equivalent looking HTML page dynamically generated in JavaScript which renders the same Chrome of the Blazor WASM App which both makes use
of the shared navigation defined in JavaScript to render their top & sidebar navigation links in a simple CSV format for easy maintenance:
...
of ServiceStack's [structured Error responses](/error-handling) heavily utilized throughout the template.
...
The Tailwind & Bootstrap components share the same functionally equivalent base classes between that can be swapped when switching CSS frameworks by updating its namespace in your projects `_Imports.razor`. 

#### Themable 

Should it be needed, their decoupled design also allows easy customization by running the included [README.ss](https://github.com/NetCoreTemplates/blazor-tailwind/blob/main/MyApp.Client/Shared/Components/README.ss) executable documentation that copies its **Razor** UI markup locally into your project enabling customization of all UI including controls.

### Bookings CRUD Example

To demonstrate ServiceStack's clean & highly productive Blazor dev model we'll [AutoQuery Bookings CRUD](/autoquery-crud-bookings) example in Blazor.

Since we're using [AutoQuery CRUD](/autoquery-crud) we only need to define the Request DTO with the input fields we want the user to fill out in our `Booking` RDBMS table in [Bookings.cs](https://github.com/NetCoreTemplates/blazor-tailwind/blob/main/MyApp.ServiceModel/Bookings.cs):
...
Where we make use of [Declarative Validation](/declarative-validation) attributes to define the validation rules for this service.
...
Thanks to ServiceStack's [recommended Project structure](/physical-project-structure) no any additional classes are needed as we're able to bind our UI Controls directly to the typed server `CreateBooking` Request DTO properties used to define the API in 
...
Calling ServiceStack APIs is just as easy, requiring no additional code-gen or boilerplate by sending a populated Request DTO with 
[JsonApiClient Api methods](/csharp-client#high-level-api-and-apiasync-methods) which encapsulate both successful API or structured error responses 
in a typed `ApiResult<T>` result.

The UI validation binding works by using Blazor's `<CascadingValue>` to propagate any `api.error` responses down to its child components.
...
Which utilizes both client and server validation when submitting an empty form, displaying UX friendly contextual errors under each field which violates our server declarative validation or client UI **required** rules:
...
We've extracted the File Upload UI component used in our [File Blazor Demo](/locode/files-blazor) into a reusable Blazor component you can utilize in your own apps, here's what it looks like on [file.locode.dev](https://file.locode.dev):
...
One of our primary goals with ServiceStack is to deliver great value and performance, features which are exemplified in our new jamstacks.net templates which are maximally designed to deliver the best performance possible in a .NET App by adopting a [Jamstack architecture](https://jamstack.org), enabling redundant hosting of their decoupled UI's on inexpensive CDN edge caches - the most optimal place to deliver the best performance. 

This also happens to be the cheapest to place to host their static files, taking load away from expensive .NET App Servers and move them to free Cloudflare or GitHub CDNs, which our Jamstack Live Demos take advantage of to enable their [**$0.40 /mo** TCO](https://jamstacks.net/posts/hosting) for their .NET Docker Apps which are only needed to serve the JSON APIs used to power their Jamstack UIs.

### Expensive Managed Databases

Having achieved the best value architecture for delivering max value and performance, the last expensive vital component used in most Web Apps is hosting of their expensive managed databases. Despite most RDBMS's being OSS and free of licensing costs, major cloud companies continue to charge artificially high hosting costs to provide redundant hosting of App data. The effect of which adds an artificial barrier to entry discouraging new Indie developers from building & hosting their dream Apps that could form self sustaining business models that should be accessible to anyone with access to a Computer with the Internet.

If we solve this, Indie developers can take advantage of our [Free Licenses](https://servicestack.net/free) and experiment building their dream Apps for a few $'s a month, instead of $100's a month.
...
Our [Jamstack live demos](https://jamstacks.net) have been able to avoid these expensive costs by using [SQLite](https://www.sqlite.org) by default, which OrmLite lets you easily change to use your [preferred RDBMS](/ormlite/installation). But when storing live customer data you'll want to ensure redundancy is available to protect against data loss and why we were excited to learn about [Litestream](https://litestream.io) for enabling effortless replicas & restore to a number of [popular storage providers](https://litestream.io/guides/).

Litestream is being developed by [@benbjohnson](https://twitter.com/benbjohnson), the creator of the popular the popular [BoltDB NoSQL embeddable DB](https://github.com/boltdb/bolt), [used by etcd](https://www.ibm.com/cloud/learn/etcd) to manage critical information distributed systems need, most notably, it manages the configuration, state and metadata for [Kubernetes](https://kubernetes.io).
...
Whilst modern RDBMS's are finely-tuned modern miracles, the one thing they don't have any control over is the latency between its distributed components. A latency SQLite doesn't have courtesy of being built-in to your application process to enable per-query latency's drop down to 10-20 microseconds - a **50-100x** improvement over intra-region RDBMS queries.
...
<section class="my-20 text-center">
    <div class="flex">
        ...
        <div class="flex flex-1 flex-col">
            <a href="https://azure.microsoft.com/en-au/services/storage/blobs/">
...
Since Litestream is tied to deployment, hosting environment & preferred configured storage, we had to create a matrix of configurations to integrate with each of the templates above.
...
These GitHub Action configurations are an effortless way to create and deploy new Applications within minutes, which only need to be filled in with your environments access credentials configured in your [projects GitHub Action Secrets](/litestream-templates.html#github-action-workflow).

## Admin UI

Another primary focus of this release was developing a suite of Admin UI tools that provide a number of high productivity features to manage users & DB Validation Rules as well as unprecendented insights into your Live running App with in-depth Request Logging & Profiling where you'll be able to observe your App's behavior in real-time.
...
First you'll be greeted with the re-designed dashboard which show high-level overview stats on the number of APIs your App has as well as internal stats counters which were previously only available in code, now they're being surfaced right on your Dashboard where they can provide valuable overview on the health of different features at a glance:

![](/images/admin-ui/dashboard-features.png)

#### Advertized features

As Admin is a capability-based UI it only shows the stats and features your App has enabled. To aid in discovery the dashboard now includes a light switch of available Admin features with a link to [Admin UI Feature Docs](/admin-ui-features), providing a summary of each feature and instructions on how to enable them. 

We'll now explore some o the new Admin UI features added in this release:

## Request Logging & Profiling

The new Request Logging & Profiling UIs bring a whole new invaluable level of observability into your App, from being able to quickly inspect and browse incoming requests, to tracing their behavior by their generated events from the new [Diagnostic Source](https://docs.microsoft.com/en-us/dotnet/api/system.diagnostics.diagnosticsource?view=net-6.0) capabilities added all throughout ServiceStack, which both power the new UIs and allows new code introspection to be able to tap in to inspect & debug when the diagnostic events occur.
...
The default configuration looks at providing useful information during development, where the response request bodies are captured in the Request Logger and the StackTrace is captured on useful events. 

### Configuration

Depending on your App you'll want to change these defaults, e.g. if you're persisting the request logs using the [CSV Request Logger](/request-logger#csv-request-logger) or [Redis Request Logger](/request-logger#redis-request-logger) it may not be feasible to **capture API responses** if they're too large. 

If enabled, **StackTraces** are captured in important events, when: 
 - **ServiceStack:** Before a Service Gateway Request is sent
 - **OrmLite:** Opening a connection or rolling back a transaction
 - **Redis:** Opening a connection, auto retrying a failed command, renting & returning clients from a redis manage pool

The latter of which is useful when resolving [concurrent usage issues](/redis/troubleshooting) from improper usage of [ServiceStack.Redis](/redis/).

In general as it adds overhead, profiling should only be added when used, e.g. during development or if needing to debug production issues. Whilst you may always want to capture request logs depending on how important it is to maintain an audit of completed API requests. Should it be needed, both plugins offer several configuration options on controlling the type, information & events captured.

Whichever features you have registered will automatically appear in the Admin UI's sidebar for quick navigation:
...
The top navigation controls which results are displayed with:
 ...
 - **Reset Filters Icon** - Clear all filters
 - **Left/Right Icons** - Navigate to previous/next pages
 - **Refresh Icon** - Reload the latest results

This same user workflow is also captured in the Profiling UI in much finer granularity, capturing all the events performed by the APIs:

![](/images/admin-ui/profiling-splash.png)

Clicking on an entry will let you inspect it in more detail, e.g. clicking on the first **/api/QueryBookings** will show the API Request and Response made:
...
The `ProfilingFeature` offers similar functionality in specifying which sources to observe and profiling events to capture as well as some options for customizing the Profiling UI:
...
Building upon ServiceStack's existing [Declarative Validation](/declarative-validation) infrastructure made it possible to seamlessly support dynamically adding declarative Type and Property Validators for each Request DTO dynamically at runtime where they'll immediately take effect and can be optionally cached where they'll only need to be re-hydrated from the database after they've been modified. 
...
This feature can be easily added to existing host projects with the [x mix script](/mix-tool) below:
...
With DB Validation support now added, we've completed our reimplementation of Studio Desktop App features within the ServiceStack library itself in its built-in [Admin UI](/admin-ui), [Locode](/locode/) and [API Explorer](/api-explorer) UIs.

![](/images/studio/studio-home.png)

Whilst Studio enabled managing multiple instances of ServiceStack from a single Desktop App, the UI not being served from the same Web App hindered development as it couldn't use integrated browser auth and forced needing to manage multiple authentication sessions for each site and required proxying every request to attach auth info. It's App Metadata also had to remain backwards-compatible to support multiple versions of ServiceStack which inhibited progress.
...
Localize Metadata
https://github.com/ServiceStack/ServiceStack/commit/0558bad9adf315f45171995ee0cb96a432820f66
https://github.com/ServiceStack/ServiceStack/commit/ec96c44d6da67779ce2e76b2314545cddd226ec9

Add ability to filter tags in ui,locode,admin-ui with ?IncludeTypes
550+ services 9.5 MB metadata
https://github.com/ServiceStack/ServiceStack/commit/fa1eff57d216f98ee7c2e33fc35e69c75ed5b854
https://forums.servicestack.net/t/api-explorer-hangs-on-large-service-layer/10743/4


## Other

Simplified ApiForm APIs
https://github.com/ServiceStack/ServiceStack/commit/4bdfd58a98de07e40c67fbd32c62a3d402b0de1c

Add TableSchemasFilter to AutoGen
https://github.com/ServiceStack/ServiceStack/commit/599ef26516d77d87f5c19ec7b7f7a468e006ccf0
https://stackoverflow.com/questions/72301160/servicestack-autoquery-table-not-accessibele#comment127739769_72304122
